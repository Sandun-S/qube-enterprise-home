...
// Generates docker-compose.yml for Swarm deployment on the Qube edge device.
// v2 changes:
//   - No CSV volume mounts — readers read from shared SQLite
//   - Bind mount /opt/qube/data for SQLite (matches conf-agent systemd path)
//   - READER_ID + SQLITE_PATH + CORESWITCH_URL env vars
//   - No MQTT broker service (removed from Qube)
//   - conf-agent NOT included — it always runs as a native binary (systemd)
//   - influxdb + core-switch included — required infrastructure for reader pipeline
...
func buildComposeYML(pool *pgxpool.Pool, qubeID, location string, containers []containerConfig) string {
	...
	coreSwitchImage := imageForService(pool, "core_switch")
	influxSQLImage := imageForService(pool, "influx_sql")
	influxRelayImage := imageForService(pool, "influx_relay")

	fmt.Fprintf(&b, `version: "3.8"
# Generated by Qube Enterprise v2 TP-API
# Qube: %s | Location: %s
# Deploy: docker stack deploy -c docker-compose.yml qube
# Note: conf-agent is NOT in this file — it runs as a systemd binary on the host.
...
services:
  ...
  # ── Core-Switch (reader data router → influxdb-relay + live) ─────────────
  core-switch:
    image: %s
    ...
    volumes:
      ...
      - /opt/qube/data:/opt/qube/data:ro
    environment:
      - HTTP_PORT=8585
      - INFLUX_URL=http://influxdb-relay:9096
      - INFLUX_DB=edgex
  ...
  # ── Enterprise Influx-to-SQL (reads influxdb → TP-API → TimescaleDB) ──────
...
`, qubeID, loc, influxRelayImage, coreSwitchImage, influxSQLImage, qubeID)
	...
	for _, c := range containers {
		...
		fmt.Fprintf(&b, "      - /opt/qube/data:/opt/qube/data:ro\n")
